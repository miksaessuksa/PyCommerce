...
if rad == "Project presentation":
    ...
    st.markdown  (">- **Trusted Shop**: verified comments, meaning they result from a client order.")
...
####################  MODELING
    ...
    st.image(img, caption=None, width=None, use_column_width=None, clamp=False, channels="RGB", output_format="auto")
    st.write("As for the deep learning models we used a dense neural network including an embeddding layer, a MaxPooling step and two dense layers. Only the reviews were used as inputs (Lemma).  \n"
...
###############     CONCLUSION  
    ...
    st.markdown("Further development of this project, on a more business oriented perspective, would be to understand the reasons behind customers satisfaction or dissatisfaction. This could help the company to monitor its activity completed with additionnal key performance indicators (KPI), tracking evolution per region of good/bad comments for each of these main categories identified.  \n"
